"""FastAPI application for serving the fine-tuned assistant."""
...
from chatbot_ft.config import load_config, resolve_project_paths
from chatbot_ft.data import build_prompt
from chatbot_ft.modeling import load_model_for_inference, load_tokenizer

app = FastAPI(title="Custom Chatbot Assistant API", version="0.1.0")


class GenerateRequest(BaseModel):
    message: str = Field(..., min_length=1, description="User prompt/instruction.")
    context: str = Field(default="", description="Optional context/input.")
...
class InferenceState:
    ...
    def load(self) -> None:
        ...
        config_path = os.getenv("CHATBOT_CONFIG_PATH", str(PROJECT_ROOT / "configs/train_config.yaml"))
        cfg = load_config(config_path)
        cfg = resolve_project_paths(cfg, project_root=PROJECT_ROOT)

        adapter_dir = os.getenv("CHATBOT_ADAPTER_DIR", str(Path(cfg.training.output_dir) / "adapter"))
        if not Path(adapter_dir).exists():
            raise FileNotFoundError(
                f"Adapter directory not found: {adapter_dir}. "
                "Train the model first or pass CHATBOT_ADAPTER_DIR."
            )
